...
def gather_variant_dataframe(variant, images_root, masks_dir):
    images_root = _norm(images_root)
    variant_dir = images_root / variant
    if not variant_dir.exists():
        raise FileNotFoundError(f"Variant directory not found: {variant_dir}")
    npy_paths = sorted(variant_dir.glob('*.npy'))
    if not npy_paths:
        raise ValueError(f"No .npy files found for variant '{variant}' under {variant_dir}")
    ...
    if not records:
        raise ValueError(f"No samples remain for variant '{variant}' after matching masks in {masks_dir}.")
    if missing_masks:
        print(f"[warn] {variant}: skipped {missing_masks} scene(s) without matching mask files.")
    return pd.DataFrame(records)

def build_internal_splits_from_train_pool(variant, images_root, masks_dir, train_val_frac, seed, max_scenes):
    df = gather_variant_dataframe(variant, images_root, masks_dir)
    if max_scenes > 0:
        scenes_subset = sorted(df['scene'].unique())[:max(1, max_scenes)]
        df = df[df['scene'].isin(scenes_subset)].copy()
    scenes = sorted(df['scene'].unique())
    if not scenes:
        raise ValueError('No scenes available after filtering variant samples.')
    rng = np.random.default_rng(seed)
    rng.shuffle(scenes)
    n_total = len(scenes)
...
def log_resolved_settings(args, df_train, df_val, run_dir, extras=None):
    ...
    if cfg['images_root']:
        print(f"  Images root:    {_short(cfg['images_root'])}")
...
def parse_args():
    parser = argparse.ArgumentParser('Config-driven training with detailed logging.')
...
def main():
    ...
    fallback_outputs = paths_cfg.get('outputs_threeband_root') or (SCRIPT_DIR / 'outputs' / 'threeband').as_posix()
    images_root = _norm(cli_args.images_root or paths_cfg.get('linear_mapped_root') or fallback_outputs)
    masks_dir = _norm(cli_args.masks_dir or paths_cfg.get('masks_dir') or (SCRIPT_DIR / 'masks'))
