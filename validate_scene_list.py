...
def build_dataframe_from_scene_list(
    variant: str,
    images_root: Path,
    masks_dir: Path,
    scenes: Sequence[str],
    ...
    missing_npys = 0
    for scene in scenes:
        npy_path = variant_dir / f"{scene}.npy"
    ...
    if missing_npys:
        print(f"[warn] Missing {missing_npys} .npy files under {variant_dir}.", file=sys.stderr)
    if missing_masks:
        print(f"[warn] Missing {missing_masks} mask files under {masks_dir}.", file=sys.stderr)
    if not records:
        raise ValueError("No matching samples were found for the requested scene list.")
...
def evaluate(model, loader, device, channels_last: bool):
...
def export_excel_metrics(metrics: Dict, excel_path: str | os.PathLike):
...
def parse_args():
    parser = argparse.ArgumentParser("Run validation on a fixed scene list using an existing checkpoint.")
    ...
    parser.add_argument("--scene-list", type=str, required=True, help="Text file listing scene ids.")
...
def main():
    ...
    scene_list = _norm(args.scene_list)
    scenes = read_scene_list(scene_list)
    df_val = build_dataframe_from_scene_list(args.variant, images_root, masks_dir, scenes)
    ...
    start = time.time()
    loss, acc, metrics, cm = evaluate(model, loader, device, channels_last=args.channels_last)
    ...
    if args.excel_out:
        export_excel_metrics(metrics, args.excel_out)
